...
class PopSimRecommender(RecommenderAlgo):
    def score(self, seeds, user_weight, global_weight, recency, exclude_rated):
        ...
        vecs = dict(zip(cat["imdb_const"], cat["vec"]))
        ...
        centroid = np.mean([vecs.get(t, np.zeros_like(next(iter(vecs.values())))) for t in base_set], axis=0)
        ...
        for _, row in pop.iterrows():
            ...
            s_user = sims.get(cid, 0.0); s_global = float(row["global_score"]); r_weight = recency_weight(row.get("year"), recency)
            score = (user_weight * s_user + global_weight * s_global) * r_weight
            scores[cid] = score
            why = []
            if s_user > 0.6: why.append("high genre overlap with your 8–10s")
            elif s_user > 0.3: why.append("moderate content similarity to your tastes")
            if s_global > 0.6: why.append("strong global acclaim")
            if recency > 0.1 and r_weight > 1.0: why.append("recent release bias applied")
